defmodule DemoWeb.PageView do
  ...
  defp char_class(socket, index) do
    cond do
      MapSet.member?(socket.assigns.incorrect_indexes, index) -> "mark-char"
      socket.assigns.index > index -> "dim-char"
      true -> ""
    end
  end

  def each_char(socket, func) do
    {_idx, _len, content} =
      ...
      |> Enum.reduce({0, 0, []}, fn word, {idx, len, acc} ->
        ...
        |> append_space(socket.assigns.index)
      end)
